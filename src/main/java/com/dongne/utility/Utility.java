...
//	  }
	...
	public static String paging2(int totalRecord, int nowPage, int recordPerPage, String category) {
		...
		int _nowPage = (nowGrp - 1) * pagePerBlock; // 10개 이전 페이지로 이동
		if (nowGrp >= 2) {
			str.append("<li class='page-item'><a class='page-link' href='./list?category=" + category + "&nowPage=" + _nowPage + "'>이전</a></li>");
		}
		...
		for (int i = startPage; i <= endPage; i++) {
			...
			if (nowPage == i) {
				...
				str.append(
						"<li class='page-item'><a class='page-link' href='./list?category=" + category + "&nowPage=" + i + "'>" + i + "</a></li>");
			}
		}

		_nowPage = (nowGrp * pagePerBlock) + 1; // 10개 다음 페이지로 이동
		if (nowGrp < totalGrp) {
			str.append("<li class='page-item'><a class='page-link' href='./list?category=" + category + "&nowPage=" + _nowPage + "'>다음</a></li>");
		}
		str.append("</ul>");
		str.append("</div>");
