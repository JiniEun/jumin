...
//			System.out.println("key : " + elem.getKey() + " , value : " + elem.getValue());
	...
	public static String paging3(int totalRecord, int nowPage, int recordPerPage, String col, String word, String districtcode) {
		int pagePerBlock = 5; // 블럭당 페이지 수
		int totalPage = (int) (Math.ceil((double) totalRecord / recordPerPage)); // 전체 페이지
		int totalGrp = (int) (Math.ceil((double) totalPage / pagePerBlock));// 전체 그룹
		int nowGrp = (int) (Math.ceil((double) nowPage / pagePerBlock)); // 현재 그룹
		...
		for (int i = startPage; i <= endPage; i++) {
			...
			if (nowPage == i) {
				...
				str.append("<li class='page-item'><a class='page-link' href='./list?districtcode="+districtcode+"&col=" + col + "&word=" + word
						+ "&nowPage=" + i + "'>" + i + "</a></li>");
			}
		}
	...
	public static String paging4(int totalRecord, int nowPage, int recordPerPage, String col, String word, String regionID) {
		int pagePerBlock = 5; // 블럭당 페이지 수
		int totalPage = (int) (Math.ceil((double) totalRecord / recordPerPage)); // 전체 페이지
		int totalGrp = (int) (Math.ceil((double) totalPage / pagePerBlock));// 전체 그룹
		int nowGrp = (int) (Math.ceil((double) nowPage / pagePerBlock)); // 현재 그룹
		...
		for (int i = startPage; i <= endPage; i++) {
			...
			if (nowPage == i) {
				...
				str.append("<li class='page-item'><a class='page-link' href='./list?regionID="+regionID+"&col=" + col + "&word=" + word
						+ "&nowPage=" + i + "'>" + i + "</a></li>");
			}
		}
...
//	  }
	...
	public static String paging2(int totalRecord, int nowPage, int recordPerPage, String category) {
		int pagePerBlock = 5; // 블럭당 페이지 수
		int totalPage = (int) (Math.ceil((double) totalRecord / recordPerPage)); // 전체 페이지
		int totalGrp = (int) (Math.ceil((double) totalPage / pagePerBlock));// 전체 그룹
		int nowGrp = (int) (Math.ceil((double) nowPage / pagePerBlock)); // 현재 그룹
		...
		int _nowPage = (nowGrp - 1) * pagePerBlock; // 10개 이전 페이지로 이동
		if (nowGrp >= 2) {
			str.append("<li class='page-item'><a class='page-link' href='./list?category=" + category + "&nowPage="
					+ _nowPage + "'>이전</a></li>");
		}
